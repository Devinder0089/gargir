...
if(!function_exists('get_home_url')) {
    function get_home_url($element=''){

    //request set path
    $set_link =(!empty($element))?$element.'/':"";
    $root=(isset($_SERVER['HTTPS']) ? "https://" : "http://") . $_SERVER['HTTP_HOST']."/";
    $root .=(explode("/",$_SERVER['PHP_SELF'])[1]);
    $root .='/'.$set_link;
...
if(!function_exists('is_permission_user')){
    function is_permission_user($meta_key){
        $ci =& get_instance();
        return $ci->permission_model->permission_by_meta_key($meta_key);
    }
}

if(!function_exists('get_permission')){
    function get_permission($mata_key,$user_id){
        $ci =& get_instance();
        return $ci->permission_model->get_data_by_mata_key_and_user_id($mata_key,$user_id);
    }
}



if(!function_exists('get_membership_expire_date')){
    function get_membership_expire_date($mid,$time_temp){
        $ci =& get_instance();
        $result='full';
        $mmbs=$ci->membership_model->get_data_row_by_id($mid);

        if(!empty($mmbs)){
            if($mmbs->validity_option == 'month'){
            $result=date('d-m-Y h:i:s',$time_temp);;
            }
        }
...
if(!function_exists('get_data_by_search_array')){
    function get_data_by_search_array($element=false,$search_element='0'){
        
        if($element){
        $explode=explode(',',$element);
        $result=array_search($search_element,$explode);
        }else{
        $result=false;
        }
...
if(!function_exists('get_convert_string_to_array')){
    function get_convert_string_to_array($element='',$key=0,$convert_string=","){

    $explode=explode($convert_string,$element);
    if(empty($explode[0])){
        unset($explode[0]);
    }
    
    $result=(isset($explode[$key]) && !empty($explode[$key]))?$explode[$key]:'0';
    return $result;
...
if(!function_exists('get_membership_row_by_id')) {
    function get_membership_row_by_id($id){
        // Get a reference to the controller object
        $ci =& get_instance();
        return $ci->membership_model->get_data_row_by_id($id);
    }
}

if(!function_exists('get_membership_service_by_allid')){
    function get_membership_service_by_allid($allid){
        $ci =& get_instance();
        return $ci->membership_service_model->get_membership_service_data_by_all($allid);
    }
}
...
if(!function_exists('get_payment_by_id')){
    function get_payment_by_id($id){
        $ci =& get_instance();
        return $ci->payment_model->get_payment_by_id($id);
    }
}
...
//check admin
if (!function_exists('is_worker')) {
    function is_worker(){
        // Get a reference to the controller object
        $ci =& get_instance();
        return $ci->auth_model->is_worker();
    }
}
...
//is_contractor
if (!function_exists('is_contractor')) {
    function is_contractor()
    {
        // Get a reference to the controller object
        $ci =& get_instance();
        return $ci->auth_model->is_contractor();
    }
}

if (!function_exists('contractor_project_check')) {
    function contractor_project_check()
    {
        // Get a reference to the controller object
        $ci =& get_instance();
        return $ci->auth_model->contractor_project_check();
    }
}
...
if (!function_exists('is_client')) {
    function is_client()
    {
        // Get a reference to the controller object
        $ci =& get_instance();
        return $ci->auth_model->is_client();
    }
}
...
if (!function_exists('message_count')) {
    function message_count(){
        ...
        $message_count=$ci->message_model->get_message_count_where(['parent_id'=>$user->id,'pin'=>'1']);
...
if (!function_exists('get_users')){
    function get_users(){
        $ci =& get_instance();
        return $ci->auth_model->get_users();
    }
}
