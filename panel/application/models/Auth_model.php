...
public function add_users_data($data){
    ...
    $data['slug']=str_slug($data['first_name']."-".uniqid());
    $data['time_temp']= $time;
    ...
    public function login(){
        ...
        $user = $this->get_user_by_email($data['email']);
            ...
            $login_id = $this->Insert_login_data($login_data);
  ...
  //update user
    ...
    public function is_contractor(){
     ...
     public function contractor_project_check(){
      $user = $this->get_logged_user();
        if($user->role == 'contractor'){
            $this->db->where('uid', $user->id);
            $this->db->where('month(created_at)', date('m'));
            $count = $this->db->get('projects')->num_rows();
            if($user->projects_allowed > $count){
                return true;
            }else{
                return false;
            }
 ...
 public function get_data_row_by_where($where){
    ...
    //get users
    public function get_users($where=false){
 ...
 //get users
    ...
    public function user_count($where=false){
        $logged_user = $this->get_logged_user(); 
         if($where){
           $this->db->where($where);
        }
        $this->db->where('status','1');
        if($logged_user->role == 'contractor'){
           $this->db->where(['contractor_id'=>$logged_user->id]); 
        }
