            ...
            <div class="col-sm-12">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped dataTable" id="cs_datatable" role="grid"
                        ...
                        $deleted = $this->db->get_where('deleted_agreements', array('agreement_id' => $agreement['id'], 'client_id' => $client->id))->row();
                           if(empty($deleted)):
                         ?>
                            <tr>
                                <td><input class="agree" type="checkbox" name="agreement_id" value="<?php echo $agreement['id']; ?>"/> <input type="checkbox" style="display:none" name="client_id" value="<?php echo $client->id; ?>"/> <?php echo html_escape($client->name); ?></td>
                                <td>
                                   <?php
                                   $signed = $this->db->get_where('customer_signed_agreements', array('agreement_id' => $agreement['id'], 'client_id' => $client->id))->row();
                                   if(empty($signed)):
                                   ?>
                                ...
                                <td>
                                   <?php
                                       ...
                                       if(empty($signed)):
                                   ?>
                                        <a data-href="<?php echo $agreement['id'] ?>" data-id="<?php echo $client->id ?>" class="sendReminder" href="javascript:void(0)">Send a reminder to Customer<i class="fa fa-envelope"></i></a>
                                    <?php else: ?>
                                        <a href="<?php echo base_url('admin')?>/customer/view/agreement/<?php echo $agreement['id']?>/<?php echo $client->id?>/<?php echo $signed->id?>" target="_blank" style="color:red">View the agreement</a>
                                    <?php endif; ?>
                                </td>
