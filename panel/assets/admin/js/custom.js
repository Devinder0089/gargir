...
function get_sub_categories(val) {
    ...
    if(MultifileUpload){
        MultifileUpload.addEventListener('change',function(){
                ...
                let MultidvPreview = document.getElementById("MultidvPreview");
                MultidvPreview.innerHTML = "";
                
                let regex = /^([a-zA-Z0-9\s_\\.\-:])+(.jpg|.jpeg|.gif|.png|.bmp)$/;
                for (var i = 0; i < MultifileUpload.files.length; i++){
                let file = MultifileUpload.files[i];
...
var Multifileuploads=document.getElementById("Multifileupload1");
  if(Multifileuploads){
        Multifileuploads.addEventListener('change',function(){
     
            if(typeof (FileReader) != "undefined"){
                
                let MultidvPreview = document.getElementById("MultidvPreview1");
                MultidvPreview.innerHTML = "";
                
                let regex = /^([a-zA-Z0-9\s_\\.\-:])+(.jpg|.jpeg|.gif|.png|.bmp)$/;
                
                for(var i = 0; i < Multifileuploads.files.length; i++){
                    let file = Multifileuploads.files[i];
